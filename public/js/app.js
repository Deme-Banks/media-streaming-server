...
async function loadMedia() {
    try {
        ...
        let fastMovies = [], fastTV = [], fastAnime = [];
        try {
            const fetchPromises = [];
            if (moviesEnabled) {
                fetchPromises.push(
        ...
        Promise.all(bgFetchPromises).then((results) => {
            let moreMovies = [], moreTV = [], moreAnime = [];
            results.forEach(result => {
    ...
    if (isAPIContent && (source !== 'local')) {
        ...
        if (streamingUrl) {
            ...
            videoInfo.innerHTML = `
                ...
                ${!currentEpInfo && media.overview ? `<p class="overview"><strong>Overview:</strong><br>${media.overview}</p>` : ''}
            `;
        } else {
