...
// Get cartoons (from multiple sources: TMDB Animation, AniList)
app.get('/api/cartoons', async (req, res) => {
  try {
    const { page = 1 } = req.query;
    const cartoons = await movieAPI.getCartoons(parseInt(page));
    console.log(`[Cartoons] Loaded ${cartoons.length} cartoons from multiple sources`);
    res.json(cartoons);
  } catch (error) {
...
// Get bulk cartoons (multiple pages for more content)
app.get('/api/bulk/cartoons', async (req, res) => {
  try {
    const { pages = 10, start = 1 } = req.query;
    const allCartoons = [];
    const startPage = parseInt(start);
    const pageCount = Math.min(parseInt(pages), 20); // Max 20 pages
    const endPage = startPage + pageCount - 1;

    console.log(`[Cartoons] Loading pages ${startPage}-${endPage} (${pageCount} pages) from multiple sources...`);

    const cartoonFetchers = [
      (p) => movieAPI.getMoviesByGenre(16, p).catch(() => []), // TMDB Animation movies
      (p) => movieAPI.getTVShowsByGenre(16, p).catch(() => []), // TMDB Animation TV
      (p) => movieAPI.getTopRatedAnimationMovies(p).catch(() => []), // Top rated animation movies
      (p) => movieAPI.getTopRatedAnimationTV(p).catch(() => []), // Top rated animation TV
      (p) => movieAPI.getPopularAnimeFromAniList(p).catch(() => []) // AniList (many cartoons)
    ];

    // Load pages in parallel batches
    const batchSize = 3;
    for (let batchStart = startPage; batchStart <= endPage; batchStart += batchSize) {
      const batchEnd = Math.min(batchStart + batchSize - 1, endPage);
      const batchPromises = [];
      ...
      for (let i = batchStart; i <= batchEnd; i++) {
        for (const fetcher of cartoonFetchers) {
          batchPromises.push(fetcher(i));
        }
      }

      const batchResults = await Promise.all(batchPromises);
      batchResults.forEach(cartoons => {
        if (Array.isArray(cartoons)) {
          allCartoons.push(...cartoons);
        }
      });

      if (batchStart % 5 === 1 || batchStart === startPage) {
        console.log(`[Cartoons] Loaded ${allCartoons.length} cartoons so far (page ${batchStart}-${batchEnd})`);
      }
    }
    ...
    console.error('Error getting bulk cartoons:', error);
    res.status(500).json({ error: 'Failed to get bulk cartoons' });
  }
});
...
app.get('/api/media/:mediaId/details', async (req, res) => {
  try {
    const { mediaId } = req.params;
    
    // Find media in combined sources
    const allContent = await movieAPI.universalSearch('', 1); // Get sample to find item
    // Actually, we need a better way - let's search by ID
    ...
    if (mediaId.startsWith('tmdb_movie_')) {
      ...
      if (movieAPI.tmdbApiKey) {
        try {
          const response = await axios.get(`https://api.themoviedb.org/3/movie/${id}`, {
            params: {
              api_key: movieAPI.tmdbApiKey,
              language: 'en-US',
          ...
          console.warn('Error fetching TMDB details:', error.message);
        }
      }
    } else if (mediaId.startsWith('tmdb_tv_')) {
      id = mediaId.replace('tmdb_tv_', '');
      ...
      if (movieAPI.tmdbApiKey) {
        try {
          ...
          console.warn('Error fetching TMDB TV details:', error.message);
        }
      }
    }
    ...
    if (movieAPI.omdbApiKey) {
      try {
        // If we have IMDb ID from TMDB, use it directly (faster and more accurate)
        if (tmdbItem.imdbId) {
          ...
          // Try searching by title and year if no IMDb ID
...
app.get('/api/streaming/:mediaId', async (req, res) => {
  try {
    ...
    // Get streaming URL with fallback support (async now)
    const streamingUrl = await movieAPI.getStreamingUrl(id, actualType, {
      season: parseInt(season),
      episode: parseInt(episode),
      language: streamLanguage,
      animeType: animeType || 'sub'
    }, false); // Don't check availability here - let frontend handle it
